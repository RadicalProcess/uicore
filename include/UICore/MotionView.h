...
namespace rp::uicore
{
    ...
    // drawn left to right. Each node is a draggable circular handle and the
    // segments between them are straight lines. The curve always spans the full
    // width because the leftmost and rightmost nodes are pinned to the side edges
    // and cannot be removed; they may only be moved vertically. Interior nodes are
    // added by clicking on an empty spot, moved by dragging, and removed by
    // shift-clicking. The default content is a single segment running from the

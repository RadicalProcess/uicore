...
namespace rp::uicore
{
    ...
    void MotionView::paint(juce::Graphics& g)
    {
        ...
        // The connecting segments, drawn as straight lines between consecutive
        // nodes.
        g.setColour(styles::foreground);
        for (auto i = static_cast<size_t>(1); i < points_.size(); ++i)
            g.drawLine(juce::Line<float>(toPixel(points_[i - 1]), toPixel(points_[i])), lineWidth_);
